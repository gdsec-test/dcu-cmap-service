...
class RequestsTransport(HttpAuthenticated):
    ...
    def open(self, request):
        resp = requests.get(request.url, data=request.message, headers=request.headers, cert=self.cert, auth=self.auth)
        return io.StringIO(resp.content.decode())

    def send(self, request):
        resp = requests.post(request.url, data=request.message, headers=request.headers, cert=self.cert, auth=self.auth)
